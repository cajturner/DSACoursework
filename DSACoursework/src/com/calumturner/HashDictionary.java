...
public class HashDictionary implements Dictionary, Iterable<String> {
	private final String AVAILABLE = "%%%%%"; // Variable to indicate slot can
												// be used, and to continue
	...
	/**
	 * HashDicitionary constructor. Initialises LoadF, hashCode, numberElemnts.
	 * 
	 * @param sH
	 *            . A instance of HashCode.
	 * @param lF
	 *            . The load factor of this Dictionary.
	 */
	 ...
	 * 
	 * @throws DictionaryException. Invalid
	 *             HashDictionary definition.
	 */
	...
	/**
	 * Inserts a word into the Dictionary. Increments totOps.Checks if value is
	 * already in dictionary and returns if true. Checks adding value will not
	 * overload the dictionary. Creates a hash code for value and probes the
	 * dictionary.
	 * 
	 * @param String
	 *            key.
	...
	/**
	 * Removes a word into the Dictionary. Increments totOps.If key's location
	 * if null throw DictionaryException. If key if found remove it and set
	 * location to AVAILABLE, decrement numElements. If for loops ends, all
	 * locations have been probed so key was not found and throw an exception.
	 * 
	 * @param String
	 *            key.
	 */
	...
	/**
	 * Searches the dictionary for String key, if location probed is null return
	 * false(Not found). If location probed value is equal to key return true.
	 * 
	 * @param String
	 *            key.
	...
	/**
	 * Increase the size of the dictionary Find the first prime number double
	 * the size of the current table size. Reinsert all the old values into the
	 * table.
	 * 
	 */
	private void increaseTable() {
		int tableSize = (table.length * 2) + 1; // Start looking for the next
												// prime number double the size
												// of the old size. Add 1 to
												// make it odd
		while (!isPrime(tableSize))
			// Recur through odd numbers till a prime number is located.
			tableSize += 2;
		String newTable[] = table;
		table = new String[tableSize]; // Set table to new prime number size
										// 0-(prime number-1).
		numElements = 0;
		for (int i = 0; i < newTable.length; i++) { // Reinsert all the odd
													// values into the new sized
													// hash table
			if (newTable[i] == null || newTable[i].equals(AVAILABLE))
				continue;
			insert(newTable[i]);
		}
	 ...
	 * @return Boolean. True if numE/dictionary size is greater than load
	 *         factor.
	 */
	private boolean isOverLoaded(int numE) {
		if ((float) numE / table.length > loadF)
			return true;
		return false;
	 ...
	 * 
	 * @param hCode
	 *            , the original compress hash code
	 * @param doubleCode
	 *            , the compressed second hash code
	 * @param j
	 *            ,
	...
	private class TableIterator implements Iterator<String> {
		...
		public boolean hasNext() {
			...
			while (nextPos <= table.length - 1) {
				if (table[nextPos] != null
						&& !(table[nextPos].equals(AVAILABLE)))
					return true;
				nextPos++;
			}
			return false;
		 ...
		 * 
		 * @return String of the next element in the array, null if no more
		 *         elements are found.
		 */
		@Override
		public String next() {
		...
		public void remove() {
			if (currentPos == -1) {
				System.out
						.println("Cannot remove until you call next() at least once.");
				return;
			}
